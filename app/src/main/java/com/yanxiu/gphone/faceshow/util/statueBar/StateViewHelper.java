...
public abstract class  StateViewHelper {
    ...
    public static final int MODE_NET_ERROR = 4;

    private int mMode = MODE_CONTENT;
    private View[] mModeViews = new View[5];
    private ViewGroup.LayoutParams[] mModeParams = new ViewGroup.LayoutParams[5];
...
//            mAnimationView.startAnimation(hyperspaceJumpAnimation);
    ...
    private void checkMode(int mode) {
        if (mode != MODE_LOADING && mode != MODE_ERROR
                && mode != MODE_EMPTY && mode != MODE_CONTENT&&mode!=MODE_NET_ERROR) {
